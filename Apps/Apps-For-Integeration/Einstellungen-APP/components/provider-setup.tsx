...
function ApiKeyProviderCard(props: ApiKeyProviderCardProps) {
  return (
    <article className="provider-card">
      ...
      <ConnectedNote state={props.state}/>
    </article>
  );
}
  ...
  return <div className="provider-setup">
    ...
    <div className="provider-grid provider-config-grid">
      <ApiKeyProviderCard
        ...
        keyInputId="openai-api-key-input"
        extraField={<><label><span>Modell</span><input value={openaiModel} onChange={(event) => setOpenaiModel(event.target.value)} placeholder={DEFAULT_OPENAI_MODEL}/></label><p className="provider-model-hint">Empfohlen für wissenschaftliche Erklärungen und mehrsprachige Übersetzungen: <code>{DEFAULT_OPENAI_MODEL}</code>. Der Modellname bleibt editierbar.</p></>}
        busy={busy === "openai"}
        onSubmit={saveOpenAI}
      ...
      <ApiKeyProviderCard
        icon="文"
        title={labels.deepl}
        description="DeepL verwendet keinen Google-Login. Free oder Pro auswählen und den eigenen API-Schlüssel einmalig speichern."
        ...
        extraField={<label><span>API-Version</span><select value={deeplTier} onChange={(event) => setDeeplTier(event.target.value as "free" | "pro")}><option value="free">DeepL API Free</option><option value="pro">DeepL API Pro</option></select></label>}
        busy={busy === "deepl"}
        onSubmit={saveDeepL}
        onRemove={() => void remove("deepl")}
        footLink={{ href: "https://www.deepl.com/account/summary", label: "DeepL-Konto öffnen" }}
        footExtra={<span>Letzter Test: {dateLabel(connections.deepl.testedAt)}</span>}

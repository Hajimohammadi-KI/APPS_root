(function (global) {
  ...
  global.addEventListener("message", function (event) {
    ...
    if (request) {
      ...
      if (event.data.type === "einstellungen:settings:error") request.reject(new Error(event.data.error || "Settings request failed"));
      else request.resolve(event.data.payload);
    }
    if (event.data.type === "einstellungen:settings:changed") listeners.forEach(function (listener) { listener(event.data.payload); });
  });
  ...
  var api = {
    ...
    get: function (options) { return send("einstellungen:settings:get", options || {}); },
    subscribe: function (listener) { listeners.add(listener); return function () { listeners.delete(listener); }; },
    open: function () { return global.open(baseUrl + "/settings", "einstellungen", "width=1180,height=860,resizable=yes,scrollbars=yes"); },
    url: baseUrl + "/settings"
  };

  global.Einstellungen = api;
})(window);

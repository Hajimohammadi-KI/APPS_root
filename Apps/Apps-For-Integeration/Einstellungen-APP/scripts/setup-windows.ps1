# Installs Einstellungen: checks Node.js, installs npm packages, builds
# the app, prepares the WSL-side copy that the web server actually runs in
# (see start-local-app.ps1 for why), creates a desktop shortcut, then
# starts the app. Plain console output on purpose -- this is meant to be
# double-clicked by someone with no technical background, so every step
...
function Invoke-Logged([string]$Title, [scriptblock]$Action) {
  ...
  # PowerShell treats each stderr line as a terminating error if it's piped
  # through a cmdlet -- so this captures via direct array assignment
  # instead (@(& cmd 2>&1), proven safe) and checks $LASTEXITCODE
  # explicitly, the same pattern Tracker's own installer uses
  # (Invoke-NpmAttempt/Invoke-BunAttempt in its setup-windows.ps1).

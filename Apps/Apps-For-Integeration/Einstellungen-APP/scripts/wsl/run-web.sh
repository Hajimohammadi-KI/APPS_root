...
# port Windows/STARTEN-WINDOWS.bat actually targets (4323). See
...
# setup-windows.ps1). This app has no separate native API process --
# everything runs through this one relay.
set -euo pipefail
...
if [ ! -x node_modules/.bin/wrangler ]; then
  echo "[run-web] node_modules missing or incomplete -- run SETUP-WINDOWS.bat again." >> "$LOG_DIR/web.log"
  exit 1
fi

...
$workRoot = Join-Path $projectRoot 'artifacts\windows-modern\.staging'
...
$expectedWorkRoot = [System.IO.Path]::GetFullPath(
  (Join-Path $projectRoot 'artifacts\windows-modern\.staging'))
...
if (-not $SkipElectronBuild) {
  Write-Host "[1/6] Building the portable Electron application..."
  ...
  Write-Host "[1/6] Reusing the existing portable Electron application..."
}
...
Write-Host "[2/6] Preparing the embedded application payload..."
...
Write-Host "[3/6] Compressing the offline payload..."
...
Write-Host "[4/6] Applying the product identity and modern visual theme..."
...
Write-Host "[5/6] Compiling the setup manager..."
...
Write-Host "[6/6] Publishing the setup file and checksum..."
...
[System.IO.File]::WriteAllText(
  "$outputFile.sha256",
  "$($hash.Hash.ToLowerInvariant())  $([System.IO.Path]::GetFileName($outputFile))`r`n",
  (New-Object System.Text.UTF8Encoding($false)))

const LOCAL_API_ORIGIN = (
  process.env.LOCAL_API_ORIGIN ?? "http://127.0.0.1:4210"
).replace(/\/$/, "");
...
async function proxyLocalApi(request: Request, context: ProxyContext) {
  ...
  if (!path.length) {
    return Response.json({ error: "Lokaler API-Pfad fehlt." }, { status: 400 });
  }
  ...
  try {
    ...
    return Response.json(
      { error: "Der lokale Deutsch-Dienst ist nicht erreichbar." },
      { status: 503 },
    );
  }
}

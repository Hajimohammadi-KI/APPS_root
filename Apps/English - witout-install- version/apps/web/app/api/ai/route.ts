	...
	try {
		...
		if (
			...
			return Response.json(
				{
					message:
						"Choose lesson content first; each text field is limited to 8000 characters.",
				},
		...
		const analysisRules =
			body.purpose === "language-analysis"
				? "Use only supplied text and rule-based NLP observations. Treat metrics as diagnostic signals, not scores. Distinguish observation from inference. Never award CEFR, mastery, automaticity, comprehension, or pronunciation; a transcript is not audio evidence. Preserve meaning. Return exactly three short sections: Observed, Repair, Transfer. Propose one explicit repair and one new-context transfer task. Do not invent missing source, audio, timing, provider checks, or history."
				: "";
		const question = [
		...
		if (!data.answer?.trim()) {
			return Response.json(
				{ message: "The connected AI provider returned no feedback." },

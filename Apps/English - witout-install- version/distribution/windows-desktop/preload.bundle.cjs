...
// distribution/windows-desktop/google-calendar-preload.cjs
var require_google_calendar_preload = __commonJS((exports2, module2) => {
  ...
  module2.exports = { registerCalendarPreload };
});

// distribution/windows-desktop/preload.cjs
var { contextBridge, ipcRenderer } = require("electron");
var {
  registerCalendarPreload
} = require_google_calendar_preload();
contextBridge.exposeInMainWorld("studyCalendar", Object.freeze({
  status: () => ipcRenderer.invoke("desktop:calendar:status"),
  connect: () => ipcRenderer.invoke("desktop:calendar:connect"),
  sync: (events) => ipcRenderer.invoke("desktop:calendar:sync", events),
  disconnect: () => ipcRenderer.invoke("desktop:calendar:disconnect")
}));
contextBridge.exposeInMainWorld("learnerProfile", Object.freeze({
  read: () => ipcRenderer.invoke("desktop:learner-profile:read"),
  merge: (update) => ipcRenderer.invoke("desktop:learner-profile:merge", update),
  replace: (profile) => ipcRenderer.invoke("desktop:learner-profile:replace", profile),
  export: () => ipcRenderer.invoke("desktop:learner-profile:export")
}));
contextBridge.exposeInMainWorld("studyAI", Object.freeze({
  status: () => ipcRenderer.invoke("desktop:ai:status"),
  configure: (configuration) => ipcRenderer.invoke("desktop:ai:configure", configuration),
  disconnect: () => ipcRenderer.invoke("desktop:ai:disconnect"),
  explain: (request) => ipcRenderer.invoke("desktop:ai:explain", request)
}));
contextBridge.exposeInMainWorld("studyDeepL", Object.freeze({
  status: () => ipcRenderer.invoke("desktop:deepl:status"),
  configure: (configuration) => ipcRenderer.invoke("desktop:deepl:configure", configuration),
  disconnect: () => ipcRenderer.invoke("desktop:deepl:disconnect"),
  translate: (request) => ipcRenderer.invoke("desktop:deepl:translate", request)
}));
registerCalendarPreload({
  ipcRenderer,
  productId: "english-grammar-automaticity",
  productName: "English Grammar Automaticity",
  storageKey: "grammar-automaticity:v27",
  locale: "en"
});
...
var locale = readArgument("desktop-locale", "en");
var darkColor = readArgument("desktop-dark", "#493465");
var highlightColor = readArgument("desktop-highlight", "#7A5AA0");
var copy = locale === "de" ? {
  action: "PDF öffnen",
  choosing: "PDF wählen …",
  opened: "Geöffnet",
...
function mountPdfLauncher() {
  ...
  icon.innerHTML = "<path d='M7 3.75h7l4 4V20.25H7z' stroke='white' stroke-width='1.7' stroke-linejoin='round'/>" + "<path d='M14 3.75v4h4M9.5 12.25h6M9.5 15.5h4.5' stroke='white' stroke-width='1.7' stroke-linecap='round'/>";
  ...
  function showStatus(message, isError = false) {
    ...
    status.style.color = isError ? "#B42318" : "#5C526B";

...
export default function Home() {
  ...
  const stepLabels = language === "de"
    ? ["Hören", "Antwort", "Replay", "Prüfen", "Korrigieren", "Verbessern", "Speichern"]
  ...
  useEffect(() => {
    ...
    const matching = conversationTopics.find((topic) =>
      (!requestedLevel || topic.level === requestedLevel) &&
      (!requestedTopic || topic.topic.toLowerCase().includes(requestedTopic) || requestedTopic.includes(topic.topic.toLowerCase()))
    ) ?? conversationTopics.find((topic) => !requestedLevel || topic.level === requestedLevel);
  ...
  function finishDailyActivity() {
    ...
    const completed = [...new Set([...(current.completedActivities || []), dailyActivity])];
  ...
  const byLevel = allForPath.filter((topic) => level === allLabel || topic.level === level);
  const skills = [allLabel, ...unique(byLevel.map((topic) => topic.skill))];
  const bySkill = byLevel.filter((topic) => skill === allLabel || topic.skill === skill);
  const categories = [allLabel, ...unique(bySkill.map((topic) => topic.category))];
  const filteredTopics = bySkill.filter((topic) => category === allLabel || topic.category === category);
  ...
  function selectPath(nextPath: (typeof paths)[number]) {
    const nextAll = copy.en.all;
    const first = conversationTopics.find((topic) => topic.path === nextPath)!;
    setPath(nextPath);
    setLevel(nextAll);
    setSkill(nextAll);
    setCategory(nextAll);
    ...
    const teacherKey =
      contextKey === selected.task
        ...
        : contextKey?.startsWith("conversation.")
          ? contextKey
          : undefined;
    const played = teacherKey ? await playTeacherAudioByContextKey(teacherKey) : false;
    if (!played) setMessage("No human recording is attached yet. Ask the teacher to add one in Teacher Studio.");
  }
  ...
  function startSpeechRecognition(initialText: string) {
    ...
    recognition.onerror = (event) => {
      if (event.error !== "aborted" && event.error !== "no-speech") {
        setMessage(`${language === "de" ? "Spracherkennung" : "Speech recognition"}: ${event.error}`);
      }
    };
    try {
      recognition.start();
  ...
  async function startRecording({ keepBaseline = false }: { keepBaseline?: boolean } = {}) {
    setMessage(null);
    if (!navigator.mediaDevices?.getUserMedia || typeof MediaRecorder === "undefined") {
      setMessage(language === "de" ? "Audioaufnahme wird von diesem Browser nicht unterstützt." : "Audio recording is not supported in this browser.");
      return;
    }
    try {
      streamRef.current?.getTracks().forEach((track) => track.stop());
      ...
      setMessage(blocked
        ? (language === "de" ? "Mikrofonzugriff wurde blockiert. Erlaube ihn im Browser und versuche es erneut." : "Microphone access was blocked. Allow it in your browser and try again.")
        : (language === "de" ? "Das Mikrofon konnte nicht gestartet werden." : "The microphone could not be started."));
    }
  }
  ...
  async function evaluateTranscript() {
    ...
    if (!reviewedTranscript) {
      setMessage(language === "de" ? "Prüfe oder ergänze zuerst dein Transkript." : "Review or enter your transcript first.");
      return;
    }
    setTranscript(reviewedTranscript);
    setInterimTranscript("");
    ...
    try {
      ...
      if (!response.ok || !isEvaluationResponse(payload)) throw new Error("Provider response was unavailable or invalid.");
  ...
  const modeCards = language === "de"
    ? [
        ["guided", "☵", "Geführtes Gespräch", "Mit Aufgabe und Korrektur üben."],
        ["challenge", "◴", "60-Sekunden-Challenge", "Flüssigkeit unter Zeitdruck trainieren."],
        ["pronunciation", "▥", "Aussprachetraining", "Aufnehmen und genau nachhören; ohne erfundene Aussprachewertung."],
      ] as const
    : [
        ["guided", "☵", "Guided conversation", "Practice with a prompt and real correction."],
        ["challenge", "◴", "60-second challenge", "Build fluency under time pressure."],
        ["pronunciation", "▥", "Pronunciation practice", "Record and replay carefully; no invented pronunciation score."],
      ] as const;
      ...
      <main>
        ...
        <section className="conversation-filter" aria-labelledby="filter-heading">
          ...
          <div className="conversation-filter-grid">
            ...
            <label><span>{text.labels[1]}</span><select disabled={recordingState !== "idle"} value={levels.includes(level) ? level : allLabel} onChange={(event) => { setLevel(event.target.value); setSkill(allLabel); setCategory(allLabel); setTopicId(""); resetSession(); }}>{levels.map((item) => <option key={item}>{item}</option>)}</select></label>
            <label><span>{text.labels[2]}</span><select disabled={recordingState !== "idle"} value={skills.includes(skill) ? skill : allLabel} onChange={(event) => { setSkill(event.target.value); setCategory(allLabel); setTopicId(""); resetSession(); }}>{skills.map((item) => <option key={item}>{item}</option>)}</select></label>
            <label><span>{text.labels[3]}</span><select disabled={recordingState !== "idle"} value={categories.includes(category) ? category : allLabel} onChange={(event) => { setCategory(event.target.value); setTopicId(""); resetSession(); }}>{categories.map((item) => <option key={item}>{item}</option>)}</select></label>
            <label><span>{text.labels[4]}</span><select disabled={recordingState !== "idle"} value={selected.id} onChange={(event) => { setTopicId(event.target.value); resetSession(); }}>{filteredTopics.map((topic) => <option value={topic.id} key={topic.id}>{topic.topic}</option>)}</select></label>
          </div>
        </section>

        <section className="mode-cards" aria-label="Practice mode">
          {modeCards.map(([mode, icon, title, description], index) => (
            <button disabled={recordingState !== "idle"} key={mode} className={`mode ${practiceMode === mode ? "active-mode" : ""}`} onClick={() => { setPracticeMode(mode); resetSession(); }}>
              <span className={index === 1 ? "orange-mode" : index === 2 ? "green-mode" : ""}>{icon}</span><div><b>{title}</b><small>{description}</small></div>{practiceMode === mode && <i>✓</i>}
            </button>
          ))}
        </section>
        ...
        <div className="workspace">
          <section className="studio">
            <div className="steps" aria-label="Practice steps">
              {stepLabels.map((step, index) => <button key={step} onClick={() => setActive(index)} className={active === index ? "active" : ""}><b>{index + 1}</b><Icon>{["♧", "♩", "▷", "▧", "✓", "↗", "▯"][index]}</Icon><span>{step}</span></button>)}
            </div>
            ...
            <div className="record-card">
              ...
              <div className="record-main">
                ...
                <div className="record-controls">
                  <button onClick={() => void audioRef.current?.play()} disabled={!audioUrl}>♧ Replay</button>
                  <button onClick={() => void startRecording()} disabled={recordingState !== "idle"}><em /> Record</button>
                  <button onClick={pauseOrResume} disabled={recordingState === "idle"}>Ⅱ {recordingState === "paused" ? "Resume" : "Pause"}</button>
                  <button onClick={finishRecording} disabled={recordingState === "idle"}>■ Stop</button>
                  <button className="primary" disabled={evaluating || (!transcript.trim() && !interimTranscript.trim())} onClick={() => void evaluateTranscript()}>✣ {evaluating ? text.evaluating : text.evaluate}</button>
                </div>
              </div>
              <div className="levels" aria-hidden="true">▂▄▆█</div>
              <label className="transcript"><b>{text.transcript}</b><textarea value={`${transcript}${interimTranscript ? ` ${interimTranscript}` : ""}`} placeholder={text.transcriptPlaceholder} onChange={(event) => { setTranscript(event.target.value); setInterimTranscript(""); setEvaluation(null); setSavedId(null); }} /></label>
            </div>

            {active === 0 && <section className="flow-panel listen-panel"><div className="flow-title"><span>♧</span><div><small>STEP 1 · {stepLabels[0]}</small><h3>{language === "de" ? "Höre die Aufgabe" : "Listen to the task"}</h3></div></div><blockquote>{selected.task}</blockquote><div className="flow-actions"><button onClick={() => speak(selected.task)}>▶ Play</button><button className="primary" onClick={() => setActive(1)}>{language === "de" ? "Ich bin bereit →" : "I’m ready →"}</button></div></section>}

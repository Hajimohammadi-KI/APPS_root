...
const ProgressInsights = dynamic(
  () =>
    import("@/features/components/progress-insights").then(
      (module) => module.ProgressInsights,
    ),
  {
    ssr: false,
    loading: () => (
...
export function DashboardScreen({
  ...
  const { state, hydrated, mutate, setTodayGrammar } = useAppStore();
  ...
  const todayUnit =
    ...
    (state.learner.selfDeclaredLevel
      ...
      : undefined);
  const hasSelectedLevel = Boolean(state.learner.selfDeclaredLevel);
  const selectedLevelUnits = state.learner.selfDeclaredLevel
    ? grammarUnits.filter(
        (unit) => unit.level === state.learner.selfDeclaredLevel,
      )
    : [];
  const selectedLevelMastery = selectedLevelUnits
    .flatMap((unit) => {
      const row = state.mastery[unit.title];
      return row && row.status !== "new" ? [row] : [];
    });
  const coverage = selectedLevelUnits.length
    ? (selectedLevelMastery.length / selectedLevelUnits.length) * 100
    : 0;
  const mastery = selectedLevelMastery.length
    ? selectedLevelMastery.reduce(
        (total, row) =>
          total +
          (row.recognitionScore +
            row.writingScore +
            row.speakingScore +
            row.repairScore +
            row.transferScore) /
            5,
        0,
      ) / selectedLevelMastery.length
    : 0;
  const automaticity = selectedLevelMastery.length
    ? selectedLevelMastery.reduce(
        (total, row) => total + row.automaticityScore,
        0,
      ) / selectedLevelMastery.length
    : 0;
  const weekActivityPoints = weekActivity.map((activity, index) => {
    const date = new Date();
  ...
  const nextStep = Math.min(3, plan.completed.length + 1);
  const continueLabel = !hasSelectedLevel
    ? "Choose level and build today’s plan"
    : progress >= 100
      ? "Review today’s practice"
      : progress === 0
        ? `Start ${dailyMinutes}-minute practice`
        : `Continue step ${nextStep} of 3`;
  return (
    <div className="page-stack">
      <section className="dashboard-hero">
          ...
          <div className="dashboard-hero-actions">
            <Button
              onClick={() => {
                ...
                document.getElementById("dashboard-cefr-level")?.focus();
              }}
              size="lg"
            >
              {hasSelectedLevel ? "Start today's path" : "Choose your level"}
        ...
        <aside className="dashboard-hero-action" aria-label="Today's practice">
          <div className="flex items-center justify-between gap-3">
            <span className="text-xs font-extrabold uppercase tracking-[0.18em] text-slate-500">
              Today's practice
            </span>
            <Badge variant="secondary">
              {todayUnit?.level ?? "Choose level"}
            </Badge>
          </div>
          ...
          {hasSelectedLevel ? (
            <>
              <div className="dashboard-practice-summary">
                <div
                  className="dashboard-practice-ring"
                  style={
                    { "--practice-progress": `${progress * 3.6}deg` } as CSSProperties
                  }
                >
                  <strong>{progress}%</strong>
                </div>
                <div>
                  <h2>{todayUnit?.title ?? "Your next practice"}</h2>
                  <p>
                    {plan.completed.length} of 3 steps complete · {dailyMinutes} minutes
                  </p>
                </div>
              </div>
              <Progress
                value={progress}
                aria-label={`Today's learning path ${progress} percent complete`}
              />
              <Button
                className="w-full justify-between active:translate-y-px"
            ...
            <div className="space-y-3">
              <div>
                <h2 className="text-xl font-bold tracking-tight text-slate-950">
                  Build your first daily practice
                </h2>
                <p className="mt-1 text-sm leading-6 text-slate-600">
                  Select your current CEFR level. You can change it later.
                </p>
              </div>
              <label
                className="block text-sm font-semibold text-slate-800"
                htmlFor="dashboard-cefr-level"
              >
                Your English level
              </label>
              <Select
                disabled={!hydrated}
                id="dashboard-cefr-level"
                name="dashboard-cefr-level"
                onChange={(event) => {
                  const level = event.target.value as CefrLevel;
                  if (!CEFR_ORDER.includes(level)) return;
                  mutate((draft) => {
                    draft.learner.selfDeclaredLevel = level;
                    draft.learner.verifiedLevel = null;
                    draft.learner.placementMode = "manual";
                    draft.learner.placementCheckedAt = null;
                  });
                  const nextGrammar = grammarUnits.find(
                    (unit) => unit.level === level,
                  );
                  if (nextGrammar) setTodayGrammar(nextGrammar);
                }}
                value=""
              >
                <option value="">
                  {hydrated ? "Select A1–C2" : "Loading your profile…"}
                </option>
              ...
              <button
                ...
                type="button"
              >
                Not sure? Open the optional level check
              </button>
            </div>
          )}
        </aside>
      ...
      <ProgressInsights
        level={state.learner.selfDeclaredLevel ?? "Not selected"}
        coverage={coverage}
        mastery={mastery}
        automaticity={automaticity}
        weekActivity={weekActivityPoints}
      />
      ...
      <div className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
        <Card className="shadow-sm">
          ...
          <CardContent>
            <Progress
              aria-label={`Weekly practice activity ${weeklyCompletion} percent`}
              value={weeklyCompletion}
            />

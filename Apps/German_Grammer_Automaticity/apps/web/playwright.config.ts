...
const localTestUrl = "http://127.0.0.1:3199";
const baseURL = externalBaseUrl ?? localTestUrl;
const pwaMode = process.env.PLAYWRIGHT_PWA === "1";
const localServerCommand = pwaMode
  ? `"${bunExecutable}" --bun next start --hostname 127.0.0.1 --port 3199`
  : `"${bunExecutable}" --bun next dev --hostname 127.0.0.1 --port 3199`;

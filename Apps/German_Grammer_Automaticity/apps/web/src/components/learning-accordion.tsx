...
export type LearningAccordionTone =
  "blue" | "violet" | "emerald" | "amber" | "rose";

const toneStyles: Record<
  LearningAccordionTone,
  { shell: string; icon: string; eyebrow: string }
> = {
  blue: {
    shell:
      "border-sky-200 bg-gradient-to-r from-sky-50/90 via-background to-background",
    icon: "bg-sky-100 text-sky-700 ring-sky-200",
    eyebrow: "text-sky-700",
  },
  violet: {
    shell:
      "border-violet-200 bg-gradient-to-r from-violet-50/90 via-background to-background",
    icon: "bg-violet-100 text-violet-700 ring-violet-200",
    eyebrow: "text-violet-700",
  },
  emerald: {
    shell:
      "border-emerald-200 bg-gradient-to-r from-emerald-50/90 via-background to-background",
    icon: "bg-emerald-100 text-emerald-700 ring-emerald-200",
    eyebrow: "text-emerald-700",
  },
  amber: {
    shell:
      "border-amber-200 bg-gradient-to-r from-amber-50/90 via-background to-background",
    icon: "bg-amber-100 text-amber-800 ring-amber-200",
    eyebrow: "text-amber-800",
  },
  rose: {
    shell:
      "border-rose-200 bg-gradient-to-r from-rose-50/90 via-background to-background",
      ...
      <AccordionItem className="border-0" value={itemValue}>
        <AccordionTrigger className="min-h-16 gap-3 px-4 py-3 text-left hover:no-underline sm:px-5">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="block text-sm font-bold leading-5 text-foreground sm:text-base">
              {title}
            </span>
            {summary ? (
        ...
        <AccordionContent className="border-t border-current/10 bg-card/85 px-4 py-4 sm:px-5">
          {children}
        </AccordionContent>
      </AccordionItem>
    </Accordion>

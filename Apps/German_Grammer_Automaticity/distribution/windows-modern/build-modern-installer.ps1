...
$workRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'DeutschFlowDesktop-windows-modern-staging'
...
$expectedWorkRoot = [System.IO.Path]::GetFullPath(
  (Join-Path ([System.IO.Path]::GetTempPath()) 'DeutschFlowDesktop-windows-modern-staging'))
...
if (-not $SkipElectronBuild) {
  ...
  Copy-Item `
    ...
    -Force
  Copy-Item `
    -LiteralPath (Join-Path $projectRoot 'apps\web\public') `
  ...
  [System.IO.File]::WriteAllText(
    ...
    (Get-FileHash -LiteralPath $webArchive -Algorithm SHA256).Hash.ToLowerInvariant(),

...
export type WerkzeugSettings = {
  ...
  planning: {
    ...
    planPausedAt: string;
    dailyWorkMode: "rescue" | "light" | "full";
...
export function normalizeWerkzeugSettings(value: unknown): WerkzeugSettings {
  ...
  const migratedSources = sources.map((source) =>
    source.id === "source-drive-research-pdfs" || source.location.includes("1WDq_TqkacdaQPHSw6kVandumb65_q90D")
      ? {
          ...source,
  ...
  return {
    ...
    planning: {
      ...
      planStartDate: planning.planStatus !== "running" && planning.planStatus !== "paused" && String(planning.planStartDate) === "2026-08-07"
        ? ""
        : /^\d{4}-\d{2}-\d{2}$/.test(String(planning.planStartDate)) ? String(planning.planStartDate) : "",
      planEndDate: planning.planStatus !== "running" && planning.planStatus !== "paused" && String(planning.planEndDate) === "2027-02-13"
        ? ""
        : /^\d{4}-\d{2}-\d{2}$/.test(String(planning.planEndDate)) ? String(planning.planEndDate) : "",

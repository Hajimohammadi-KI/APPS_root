...
**Scope:** `Apps/English/English-07082026` only, per the contract's own stated scope. No files were changed as part of this audit.
...
Reordered from the draft by evidence, not by restating it — the real blocker set is smaller than 10 phases suggests once you see that F1's fix is "wire up an existing function" rather than "build a verification engine."

### P0 — Unblocks Release 1 (est. days, not weeks, given F1's infrastructure already exists)
...
### P1 — Unblocks Release 2 (real work, correctly scoped as bigger in the original draft)
...
## What I'd want confirmed before touching code
This is a findings + roadmap document only, per the request — nothing above has been implemented. F1 and F4 are both small, well-contained changes with a proven pattern to copy from the same codebase; F2 and F5 are genuine new-feature work. Say which of these you want done first.
